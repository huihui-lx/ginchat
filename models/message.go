...
func Chat(writer http.ResponseWriter, request *http.Request) {
	...
	//7.加入在线用户到缓存
	onlineTime := time.Duration(viper.GetInt("timeout.RedisOnlineTime"))*time.Hour
	SetUserOnlineInfo("online_"+Id, []byte(node.Addr), onlineTime)
...
func sendProc(node *Node) {
	for {  //无限循环，不断处理消息发送操作。
		...
		case data := <-node.DataQueue:
			...
			//使用 WebSocket 连接 node.Conn 发送消息。它调用了 WriteMessage 方法，并指定消息类型为 websocket.TextMessage，然后将消息数据 data 发送出去
